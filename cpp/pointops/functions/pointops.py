...
furthestsampling = _allow_in_graph(FurthestSampling.apply)
...
knnquery = _allow_in_graph(KNNQuery.apply)
...
ballquery = _allow_in_graph(BallQuery.apply)
...
class Grouping(Function):
    ...
    def forward(ctx, input, idx):
        ...
        output = torch.empty((m, nsample, c), device=input.device, dtype=torch.float32)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros((n, c), device=grad_output.device, dtype=torch.float32)
        pointops_cuda.grouping_backward_cuda(
            m, nsample, c, grad_output, idx, grad_input)
        return grad_input, None


grouping = _allow_in_graph(Grouping.apply)
...
class Subtraction(Function):
    ...
    def forward(ctx, input1, input2, idx):
        ...
        n, c = input1.shape
        nsample = idx.shape[-1]
        output = torch.zeros((n, nsample, c), device=input1.device, dtype=torch.float32)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input1 = torch.zeros((n, c), device=grad_output.device, dtype=torch.float32)
        grad_input2 = torch.zeros((n, c), device=grad_output.device, dtype=torch.float32)
        pointops_cuda.subtraction_backward_cuda(
            n, nsample, c, idx, grad_output, grad_input1, grad_input2)
        return grad_input1, grad_input2, None


subtraction = _allow_in_graph(Subtraction.apply)
...
class Aggregation(Function):
    ...
    def forward(ctx, input, position, weight, idx):
        ...
        w_c = weight.shape[-1]
        output = torch.zeros((n, c), device=input.device, dtype=torch.float32)
    ...
    def backward(ctx, grad_output):
        ...
        w_c = weight.shape[-1]
        grad_input = torch.zeros((n, c), device=grad_output.device, dtype=torch.float32)
        grad_position = torch.zeros((n, nsample, c), device=grad_output.device, dtype=torch.float32)
        grad_weight = torch.zeros((n, nsample, w_c), device=grad_output.device, dtype=torch.float32)
...
aggregation = _allow_in_graph(Aggregation.apply)
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    new_feat = torch.zeros((new_xyz.shape[0], feat.shape[1]), device=feat.device, dtype=torch.float32)
    for i in range(k):
...
class Interpolation(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros((m, c), device=grad_output.device, dtype=torch.float32)
...
interpolation2 = _allow_in_graph(Interpolation.apply)
